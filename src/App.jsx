import { useState, useEffect } from 'react'
import { supabase } from './supabaseClient'

function App() {
  const [name, setName] = useState('')
  const [queue, setQueue] = useState([])
  const [currentGame, setCurrentGame] = useState([])
  const [elapsedTime, setElapsedTime] = useState('0:00')
  const [showResults, setShowResults] = useState(false)

  const myLocalName = localStorage.getItem('fauHoopsName') || ''

  // 1. Initial Load & Real-time Subscriptions
  useEffect(() => {
    fetchData()

    const queueChannel = supabase.channel('q-sync')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'queue' }, () => fetchData())
      .subscribe()

    const gameChannel = supabase.channel('g-sync')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'current_game' }, () => fetchData())
      .subscribe()

    return () => {
      supabase.removeChannel(queueChannel)
      supabase.removeChannel(gameChannel)
    }
  }, [])

  // 2. Persistent Timer Logic (Syncs to DB Timestamp)
  useEffect(() => {
    const interval = setInterval(() => {
      if (currentGame.length === 2) {
        // We use the 2nd person's join time so the timer starts when the MATCH is set
        const startTime = new Date(currentGame[1].joined_at).getTime()
        const now = new Date().getTime()
        const diff = Math.floor((now - startTime) / 1000)

        if (diff > 0) {
          const mins = Math.floor(diff / 60)
          const secs = diff % 60
          setElapsedTime(`${mins}:${secs < 10 ? '0' : ''}${secs}`)
        } else {
          setElapsedTime('0:00')
        }
      } else {
        setElapsedTime('0:00')
      }
    }, 1000)
    return () => clearInterval(interval)
  }, [currentGame])

  // 3. Main Data Fetch & Auto-Promotion
  const fetchData = async () => {
    const { data: qData } = await supabase.from('queue').select('*').order('created_at', { ascending: true })
    const { data: gData } = await supabase.from('current_game').select('*').order('joined_at', { ascending: true })

    const currentQ = qData || []
    const currentG = gData || []

    setQueue(currentQ)
    setCurrentGame(currentG)

    // Check if I'm still in the system. If deleted by others, unlock device.
    const stillInSystem = currentQ.some(p => p.name === myLocalName) || currentG.some(p => p.player_name === myLocalName)
    if (!stillInSystem && myLocalName) {
      localStorage.removeItem('fauHoopsName')
      localStorage.removeItem('fauHoopsJoined')
    }

    // AUTO-PROMOTION: Court has space? Move the next person up.
    if (currentG.length < 2 && currentQ.length > 0) {
      const nextPlayer = currentQ[0]
      try {
        // This might fail if another device beats us to it, which is fine!
        const { error: insertError } = await supabase.from('current_game').insert([{ player_name: nextPlayer.name }])
        if (!insertError) {
          await supabase.from('queue').delete().eq('id', nextPlayer.id)
        }
      } catch (e) {
        console.log("Promotion handled by another instance.")
      }
    }
  }

  // 4. Join the Queue
  const joinQueue = async (e) => {
    e.preventDefault()
    const cleanName = name.trim()
    if (!cleanName) return

    // Pre-check for duplicate names (UI layer)
    const exists = queue.some(p => p.name.toLowerCase() === cleanName.toLowerCase()) ||
                   currentGame.some(p => p.player_name.toLowerCase() === cleanName.toLowerCase())

    if (exists) {
      alert("This name is already in the system!")
      return
    }

    const { error } = await supabase.from('queue').insert([{ name: cleanName }])

    if (error) {
      if (error.code === '23505') alert("Name already taken!")
      else alert("Join failed: " + error.message)
    } else {
      localStorage.setItem('fauHoopsName', cleanName)
      localStorage.setItem('fauHoopsJoined', 'true')
      setName('')
    }
  }

  // 5. Leave / Finish Logic
  const leaveEverything = async () => {
    if (!window.confirm("Are you sure you want to leave?")) return
    await supabase.from('queue').delete().eq('name', myLocalName)
    await supabase.from('current_game').delete().eq('player_name', myLocalName)
    localStorage.removeItem('fauHoopsName')
    localStorage.removeItem('fauHoopsJoined')
    setShowResults(false)
  }

  const resolveGame = async (loserId) => {
    // Loser is deleted. Winner remains in current_game.
    // The fetchData loop will then see a free spot and pull the next player.
    const { error } = await supabase.from('current_game').delete().eq('id', loserId)
    if (!error) setShowResults(false)
  }

  // State Checks
  const amIInGame = currentGame.some(p => p.player_name === myLocalName)
  const amIInQueue = queue.some(p => p.name === myLocalName)
  const isRegistered = amIInGame || amIInQueue

  return (
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #001f3f 0%, #003366 100%)', padding: '20px 10px', color: '#fff', fontFamily: 'Inter, -apple-system, sans-serif' }}>
      <div style={{ maxWidth: '480px', margin: '0 auto' }}>

        <header style={{ textAlign: 'center', padding: '20px 0' }}>
          <h1 style={{ margin: 0, fontSize: '24px', fontWeight: '900', letterSpacing: '-1px' }}>
            FAU <span style={{ color: '#CC0000' }}>HOOPS</span>
          </h1>
          <div style={{ fontSize: '10px', opacity: 0.5, letterSpacing: '1px' }}>BOCA RATON • RECREATION</div>
        </header>

        {/* COURT SECTION */}
        <div style={{
          background: 'rgba(255, 255, 255, 0.05)',
          backdropFilter: 'blur(10px)',
          padding: '25px',
          borderRadius: '24px',
          border: '2px solid #CC0000',
          marginBottom: '20px',
          textAlign: 'center',
          boxShadow: '0 10px 30px rgba(0,0,0,0.3)'
        }}>
          <div style={{ fontSize: '10px', fontWeight: '900', color: '#CC0000', letterSpacing: '2px', marginBottom: '15px' }}>ON COURT</div>

          <div style={{ display: 'flex', justifyContent: 'space-around', alignItems: 'center', marginBottom: '15px' }}>
            {currentGame.map(p => (
              <div key={p.id} style={{ textAlign: 'center' }}>
                <div style={{ fontSize: '18px', fontWeight: '700' }}>{p.player_name}</div>
                {p.player_name === myLocalName && <div style={{ fontSize: '10px', color: '#CC0000', fontWeight: 'bold' }}>YOU</div>}
              </div>
            ))}
            {currentGame.length < 2 && <div style={{ opacity: 0.3, fontSize: '14px' }}>Waiting for match...</div>}
          </div>

          <div style={{ fontSize: '36px', fontWeight: '800', marginBottom: '20px', fontFamily: 'monospace' }}>
            {elapsedTime}
          </div>

          {amIInGame && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              {!showResults ? (
                <button onClick={() => setShowResults(true)} style={{ width: '100%', padding: '15px', backgroundColor: '#CC0000', color: 'white', border: 'none', borderRadius: '12px', fontWeight: 'bold', cursor: 'pointer' }}>
                  FINISH GAME
                </button>
              ) : (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                  <p style={{ fontSize: '12px', opacity: 0.7 }}>Select the winner:</p>
                  <div style={{ display: 'flex', gap: '8px' }}>
                    {currentGame.map(p => (
                      <button
                        key={p.id}
                        onClick={() => resolveGame(currentGame.find(other => other.id !== p.id).id)}
                        style={{ flex: 1, padding: '12px', backgroundColor: p.player_name === myLocalName ? '#28a745' : '#444', color: 'white', border: 'none', borderRadius: '10px', fontWeight: 'bold', cursor: 'pointer' }}
                      >
                        {p.player_name === myLocalName ? "I WON" : `${p.player_name} WON`}
                      </button>
                    ))}
                  </div>
                </div>
              )}
              <button onClick={leaveEverything} style={{ background: 'none', border: 'none', color: '#666', fontSize: '12px', textDecoration: 'underline', cursor: 'pointer', marginTop: '5px' }}>
                Leave Court
              </button>
            </div>
          )}
        </div>

        {/* JOIN / STATUS SECTION */}
        {isRegistered ? (
          <div style={{ background: 'rgba(0,0,0,0.2)', padding: '20px', borderRadius: '20px', textAlign: 'center', marginBottom: '20px', border: '1px solid rgba(255,255,255,0.1)' }}>
            <div style={{ fontSize: '14px', fontWeight: 'bold' }}>You are currently <span style={{ color: '#CC0000' }}>{amIInGame ? "ON COURT" : "IN LINE"}</span></div>
            {!amIInGame && (
              <button onClick={leaveEverything} style={{ background: 'none', border: 'none', color: '#888', fontSize: '12px', textDecoration: 'underline', marginTop: '8px', cursor: 'pointer' }}>
                Remove me from list
              </button>
            )}
          </div>
        ) : (
          <div style={{ background: 'rgba(255,255,255,0.05)', padding: '20px', borderRadius: '24px', marginBottom: '25px' }}>
            <form onSubmit={joinQueue} style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              <label style={{ fontSize: '10px', fontWeight: 'bold', opacity: 0.5, marginLeft: '5px' }}>PLAYER NAME</label>
              <div style={{ display: 'flex', gap: '10px' }}>
                <input
                  value={name}
                  onChange={e => setName(e.target.value)}
                  placeholder="Enter handle..."
                  style={{ flex: 1, padding: '15px', borderRadius: '12px', border: 'none', background: 'rgba(255,255,255,0.1)', color: '#fff', outline: 'none' }}
                />
                <button type="submit" style={{ padding: '0 25px', backgroundColor: '#CC0000', color: 'white', border: 'none', borderRadius: '12px', fontWeight: 'bold', cursor: 'pointer' }}>
                  JOIN
                </button>
              </div>
            </form>
          </div>
        )}

        {/* WAITLIST SECTION */}
        <div style={{ padding: '0 5px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '15px', opacity: 0.6 }}>
            <h3 style={{ fontSize: '14px', margin: 0, fontWeight: '700' }}>WAITLIST</h3>
            <span style={{ fontSize: '14px' }}>{queue.length} Players</span>
          </div>

          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            {queue.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '30px', border: '1px dashed rgba(255,255,255,0.1)', borderRadius: '15px', color: '#555' }}>
                List is empty.
              </div>
            ) : (
              queue.map((p, i) => (
                <div key={p.id} style={{ padding: '15px 20px', background: 'rgba(255,255,255,0.05)', borderRadius: '15px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span style={{ fontWeight: '600', fontSize: '16px' }}>{i + 1}. {p.name}</span>
                  {p.name === myLocalName && <span style={{ fontSize: '10px', color: '#CC0000', fontWeight: 'bold' }}>YOU</span>}
                </div>
              ))
            )}
          </div>
        </div>

        {/* ADMIN RESET (HIDDEN) */}
        <footer
          onClick={async () => {
            if (window.confirm("Admin: Clear all court data?")) {
              await supabase.from('queue').delete().neq('id', '00000000-0000-0000-0000-000000000000');
              await supabase.from('current_game').delete().neq('id', '00000000-0000-0000-0000-000000000000');
              localStorage.clear();
              window.location.reload();
            }
          }}
          style={{ textAlign: 'center', opacity: 0.1, fontSize: '10px', padding: '50px 0', cursor: 'pointer' }}
        >
          FAU HOOPS • ADMIN RESET
        </footer>

      </div>
    </div>
  )
}

export default App